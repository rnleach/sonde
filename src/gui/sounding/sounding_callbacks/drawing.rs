...
pub fn draw_wind_profile(cr: &Context, ac: &AppContext) {
    // TODO: Make the wind profile its own module. It's complicated.
    ...
    let (shaft_length, _barb_length) = cr.device_to_user_distance(SHAFT_LENGTH_IN_PIXELS, BARB_LENGTH_IN_PIXELS);
    ...
    let mut last_kept: ScreenRect = ScreenRect{lower_left: (::std::f64::MAX,::std::f64::MAX), upper_right: (::std::f64::MAX,::std::f64::MAX)};
    for (d, s, center, rect) in barb_data {
        if rect.overlaps(&last_kept) { continue; }
        last_kept = rect;
...
fn get_wind_barb_center( pressure: f64, xcenter: f64, ac: &AppContext) -> ScreenCoords {
    ...
    let (_, yc) = ac.convert_tp_to_screen((0.0, pressure));
    
    (xcenter, yc)
}

fn get_wind_barb_box(center: ScreenCoords, direction: f64, _speed: f64, shaft_length: f64, cr: &Context) -> ScreenRect {
    ...
    if lower_left.0 > shaft_end.0 {lower_left.0 = shaft_end.0;}
    if upper_right.0 < shaft_end.0 { upper_right.0 = shaft_end.0;}
    if lower_left.1 > shaft_end.1 {lower_left.1 = shaft_end.1;}
    if upper_right.1 < shaft_end.1 { upper_right.1 = shaft_end.1;}

    // TODO: take barbs and penants into account

    ScreenRect {lower_left, upper_right}
}

fn draw_wind_barb(direction: f64, _speed: f64, shaft_length: f64, center: ScreenCoords, cr: &Context) {
    
    
    let (dot_size, _) = cr.device_to_user_distance(6.0,6.0);
    cr.arc(center.0, center.1, dot_size, 0.0, 2.0 * ::std::f64::consts::PI);
    cr.fill();
