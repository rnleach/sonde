...
impl FirePlumeContext {
    ...
    pub fn convert_xy_to_dth(coords: XYCoords) -> DtHCoords {
        ...
        let dt = CelsiusDiff(x * (max_dt - min_dt) + min_dt);
...
impl Drawable for FirePlumeContext {
    ...
    fn set_up_drawing_area(acp: &AppContextPointer) -> Result<(), SondeError> {
        let da: DrawingArea = acp.fetch_widget("fire_plume_area")?;
        ...
        let ac = Rc::clone(acp);
        da.connect_configure_event(move |_da, ev| ac.fire_plume.configure_event(ev));
    ...
    fn build_legend_strings(ac: &AppContext) -> Vec<(String, Rgba)> {
        ...
        lines.push(("Fire Plume Stability".to_owned(), config.label_rgba));
        lines.push(("Equilibrium Level".to_owned(), config.fire_plume_el_color));
        lines.push(("Max Plume Height".to_owned(), config.fire_plume_maxh_color));
